...
export default function MediaLibrary({ selectedSite }: { selectedSite: Site }) {
  ...
  // Update images when selectedSite.mediaLibrary changes
  useEffect(() => {
    setImages(selectedSite.mediaLibrary || []);
  }, [selectedSite.mediaLibrary]);

  // Memoize the handleImageSelect function to avoid recreating it on every render
      ...
      if (image) {
        ...
        try {
          const response = await axios.put(
            `/api/site/${selectedSite.path}/media-library`,
            { urls: [image] }
          );
    ...
    <div>
      ...
      {images.length > 0 ? (
        ...
        <div className="flex flex-col gap-12 w-full h-full items-center my-12">
          <h1 className="uppercase text-4xl font-bold text-stone-800 dark:text-white">
